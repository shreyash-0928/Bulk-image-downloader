...
function Results() {
  ...
  useEffect(() => {
    setIsLoading(true);
    fetch(
      `https://api.unsplash.com/search/collections/?per_page=20&client_id=${key}&query=${searchTerm}`
    )
      ...
      {!isLoading && (
        <main className="results">
          <p className="resultName">
            Showing results for <span className="product">{searchTerm}</span>
          </p>
          {error && <ErrorComponent />}

          <div className="pack-container">{collectionPack}</div>
        </main>
      )}
