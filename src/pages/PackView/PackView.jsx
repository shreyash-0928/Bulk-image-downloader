...
function PackView() {
  ...
  const [title, setTitle] = useState(() =>
    localStorage.getItem("images-title")
  );
  ...
  useEffect(() => {
    fetch(
      `https://api.unsplash.com/collections/${collectionId}/photos?client_id=${key}&per_page=30`
    )
  ...
  function handleImageDownload() {
    return fetch(
      `https://api.unsplash.com/collections/${collectionId}/photos?client_id=${key}&page=${page}&per_page=${perPage}`
    )
      .then((response) => response.json())
      .then((data) => {
        images.push(...data);
      ...
      .catch((err) => {
        alert(
          "Seems like an error ocurred while trying to download, please check your internet connection and try again"
        );
      });
  }
    ...
    <div className="bg">
      ...
      {!isLoading && (
        <main className="pack-view">
          ...
          <div className="pack-images">{collectionPhotos}</div>

          {!error && (
            <button onClick={handleImageDownload} className="download-button">
      ...
      {downloadIndicator == true && (
        <div className="download-indicator hidden">
